...
class DarkChatbotGUI:
    def __init__(self, root):
        ...
        try:
            self.root.iconbitmap("icon/chat.ico")
        except:
            pass
...
• QA Groups: {qa_groups_count}
• Streaming Speed: {config['streaming_speed']} WPM
• Streaming Mode: {'Letter-by-Letter' if config['letter_streaming'] else 'Word-by-Word'}
• Additional Info Speed: {config['additional_info_speed']} WPM
• Confidence Requirement: {config['confidence_requirement']:.2f}
• Speed Limiting: {'Enabled' if config['speed_limit'] else 'Disabled'}
